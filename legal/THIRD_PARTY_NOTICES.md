# 第三方权利与来源

Moonwild 是用于非商业学习交流的非官方同人游戏。项目的 PolyForm Noncommercial / CC BY-NC 许可只覆盖贡献者有权授权的原创部分，不改变下列内容的原有权利或许可。

## 《八方旅人》系列人物、精灵与商标

游戏采用《Octopath Traveler II》《Octopath Traveler: Champions of the Continent》以及《Octopath Traveler 0》的角色或怪物精灵。相关角色、游戏名称、图像和商标归 SQUARE ENIX 及各原权利人所有；项目不宣称它们为原创、公共领域或 CC 素材，也不提供针对这些内容的再授权。

| 内容 | 工程内位置 | 具体来源、上传者及文件记录 |
|---|---|---|
| 欧休提 OT2 战斗与弓分层素材 | `assets/sprites/reference/ochette/` | [原始来源记录](https://github.com/liny0219/moonwild/blob/main/docs/ASSET_SOURCES.md)、[文件清单](https://github.com/liny0219/moonwild/blob/main/assets/sprites/reference/ochette/source_manifest.json) |
| 洁卡莉特EX／Sigrid EX身体与Default／Crystal Spear分层源 | `assets/sprites/cotc/`、`assets/sprites/reference/jeclyte/`及v1／v2／v3开发运行派生资源 | [原始来源与开发装配记录](https://github.com/liny0219/moonwild/blob/main/docs/ASSET_SOURCES.md)；守护／引导的角度、手型和时序是项目装配，不是原作官方动作；角色与武器原设计权利不因分层编译改变 |
| 大陆霸者角色与学者猫莱拉 | `assets/sprites/cotc/` | [角色说明](https://github.com/liny0219/moonwild/blob/main/docs/COTC_ASSETS.md)、[来源清单](https://github.com/liny0219/moonwild/blob/main/assets/sprites/cotc/sources.json) |
| 普通怪物、精英与章节首领精灵 | `assets/sprites/reference/` 的其余原作目录 | [怪物说明](https://github.com/liny0219/moonwild/blob/main/docs/ORIGINAL_MONSTERS.md)、[来源清单](https://github.com/liny0219/moonwild/blob/main/assets/sprites/reference/sources.json) |

The Spriters Resource 是本项目记录的社区归档来源，不是这些游戏素材的权利人。保留对 Dark_Ansem、laceelincia、MtfujiInMyPants、Winmillion21 等具体归档上传者的记录；完整人员名单以逐项来源清单为准。下载页、上传记录或本仓库的公开状态不构成向接收者授予原作素材使用权。

同人剧情、黑化设定和项目制作的运行时编舞不等于原作官方设定。截图、录屏和 QA 图片可能同时包含项目内容与原作素材，不能将整张图片或整段视频一概标记为项目 CC 许可。公开使用这些混合内容时，分别保留项目与原作来源，并遵循相关权利人的适用规则。

空心卫兵与黑曜书卫的六姿态攻击（`assets/sprites/derived/*_attack_v2.png`）由项目使用内置图像生成工具参考原怪物设计二创，并经像素动作审阅修订。原始待机精灵保留不变；新攻击并非原作官方动画，也不因重新绘制而转移或消除原角色设计的第三方权利。

## 字体

`assets/pixel.ttf` 使用 Fusion Pixel Font，并作为完整母版保留。运行子集 `assets/fonts/pixel_ui.ttf` 由该母版派生，修改后的字体名为 Moonwild Pixel UI；保留当前文本的原字形与度量，继续适用 SIL OFL 1.1。保留以下随项目已有的版权与许可文件；项目非商业许可不限制字体许可原本允许的使用。

| 字体或组成来源 | 作者 / 原权利声明 | 保留的原文 |
|---|---|---|
| Moonwild Battle UI 480（Noto Sans SC 2.004-H2静态子集） | (c) 2014–2021 Adobe，原保留名Source；修改后字体已另命名 | [SIL OFL 1.1](assets/fonts/OFL.txt)、[母版hash／生成与覆盖](docs/BATTLE_UI_FONT.md) |
| Fusion Pixel Font | Copyright (c) 2022, TakWolf | [SIL OFL 1.1](https://github.com/liny0219/moonwild/blob/main/assets/FONT-LICENSE.txt) |
| Ark Pixel Font | Copyright (c) 2021, TakWolf | [SIL OFL 1.1](https://github.com/liny0219/moonwild/blob/main/assets/FONT-LICENSES/ark-pixel/OFL.txt) |
| Galmuri | Copyright (c) 2019–2025 Lee Minseo | [SIL OFL 1.1](https://github.com/liny0219/moonwild/blob/main/assets/FONT-LICENSES/galmuri/LICENSE.txt) |
| Cubic 11 及其中列明的位图字体来源 | 各原文所列作者，包括 M+ FONTS PROJECT / COZ | [既有许可与署名原文](https://github.com/liny0219/moonwild/blob/main/assets/FONT-LICENSES/cubic-11/OFL.txt) |

Cubic 11 目录中的文件名为 `OFL.txt`，其内容包含各来源自己的条款；不能仅凭文件名把所有组成部分重新归为 OFL 或 CC。

## Godot Engine

游戏由 Godot 4.5.1 构建，Godot Engine 采用 MIT 许可。Godot 的版权与许可不因 Moonwild 的非商业定位发生变化。

- Copyright (c) 2014-present Godot Engine contributors.
- Copyright (c) 2007-2014 Juan Linietsky, Ariel Manzur.
- [Godot 官方许可与版权声明](https://godotengine.org/license/)
- [4.5.1 引擎许可正文](https://github.com/godotengine/godot/blob/4.5.1-stable/LICENSE.txt)
- [4.5.1 引擎所用第三方组件声明](https://github.com/godotengine/godot/blob/4.5.1-stable/COPYRIGHT.txt)

分发引擎或包含引擎的游戏包时，保留引擎及所用组件的适用声明。以上官方链接可随项目文档与分发说明一同提供，不把引擎二进制重新标为 PolyForm 或 CC。

## 项目单独制作的内容

[环境图记录](https://github.com/liny0219/moonwild/blob/main/docs/GENERATED_ART.md)列出的环境图由图像生成工具生成；[音频清单](https://github.com/liny0219/moonwild/blob/main/assets/audio/manifest.json)将 15 个 WAV 记录为项目确定性合成音频。它们与原作精灵分开记录。对应项目原创权利的 CC BY-NC 4.0 适用范围见 [LICENSING.md](https://github.com/liny0219/moonwild/blob/main/docs/LICENSING.md)，不借此宣称对不存在或不属于项目的权利拥有所有权。

新增素材、字体、依赖或来源变化时，应同步更新此文件和对应清单。未列明、来源不明或许可未明的第三方文件不能自动视为已获得项目许可。

The six-pose wolf biting sheets under `assets/sprites/derived/` are AI-assisted fan-work variations referencing the original wolf designs. Original source atlases remain unchanged. These new poses are not claimed to be original game attack frames, and no rights to the underlying designs are granted by the project licenses.

The forest-stag body and lateral-antler sheets (`stag_actions_v3.png`, `stag_sweep_v2.png`) are AI-assisted fan-work poses based on the unchanged original forest-deer design. They supply this fan game's horn strike, lateral sweep, hoof stomp and grounded preparation, and are not extracted official attack animations. Original artwork/design rights remain with their respective owners; the project licenses do not relicense those third-party designs.

The stone-dragon breath sheet (`assets/sprites/derived/drake_breath_v3.png`) is an AI-assisted fan-work variation based on the unchanged original dragon design. Its nine inhaling, charging, breathing and recovering poses were created with the built-in image generation tool and are not extracted official attack frames. The project licenses do not grant rights to the underlying third-party design.

The thorn-boar body sheet (`assets/sprites/derived/boar_surge_v1.png`) contains six AI-assisted fan-work poses based on the unchanged original boar design. The built-in image generation tool supplied bracing, charging, striking and recovery anatomy; these are not extracted official attack animations. The project licenses do not grant rights to the underlying third-party design.
